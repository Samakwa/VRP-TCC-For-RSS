...
def ind2route(individual, cluster):
    route = []
    vehicleCapacity = cluster['vehicle_capacity']
    deportDueTime =  cluster['deport']['due_time']
    ...
    for customerID in individual:
        # Update vehicle load
        demand = cluster['customer_%d' % customerID]['demand']
        updatedVehicleLoad = vehicleLoad + demand
        # Update elapsed time
        serviceTime = cluster['customer_%d' % customerID]['service_time']
        returnTime = cluster['distance_matrix'][customerID][0]
        updatedElapsedTime = elapsedTime + cluster['distance_matrix'][lastCustomerID][customerID] + serviceTime + returnTime
        ...
        if (updatedVehicleLoad <= vehicleCapacity) and (updatedElapsedTime <= deportDueTime):
            ...
            subRoute = [customerID]
            vehicleLoad = demand
            elapsedTime = cluster['distance_matrix'][0][customerID] + serviceTime
...
def evalVRPTW(individual, cluster, unitCost=1.0, initCost=0, waitCost=0, delayCost=0):
    totalCost = 0
    route = ind2route(individual, cluster)
    totalCost = 0
    for subRoute in route:
        subRouteTimeCost = 0
        subRouteDistance = 0
        ...
        for customerID in subRoute:
            ...
            distance = cluster['distance_matrix'][lastCustomerID][customerID]
            # Update sub-route distance
            subRouteDistance = subRouteDistance + distance
            # Calculate time cost
            arrivalTime = elapsedTime + distance
            timeCost = waitCost * max(cluster['customer_%d' % customerID]['ready_time'] - arrivalTime, 0) + delayCost * max(arrivalTime - cluster['customer_%d' % customerID]['due_time'], 0)
            # Update sub-route time cost
            subRouteTimeCost = subRouteTimeCost + timeCost
            # Update elapsed time
            elapsedTime = arrivalTime + cluster['customer_%d' % customerID]['service_time']
            # Update last customer ID
            lastCustomerID = customerID
        # Calculate transport cost
        subRouteDistance = subRouteDistance + cluster['distance_matrix'][lastCustomerID][0]
...
def gaVRPTW(instName, unitCost, initCost, waitCost, delayCost, indSize, popSize, cxPb, mutPb, NGen, exportCSV=False, customizeData=False):
    ...
    toolbox.register('evaluate', evalVRPTW, cluster=cluster, unitCost=unitCost, initCost=initCost, waitCost=waitCost, delayCost=delayCost)
    ...
    printRoute(ind2route(bestInd, cluster))
